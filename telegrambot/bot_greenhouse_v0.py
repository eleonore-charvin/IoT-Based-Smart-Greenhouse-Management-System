...
class GreenhouseBot:
    ...
    def cmd_create_user(self, chat_id, args):
        ...
        r = requests.post(f"{API_BASE}/users", json=payload, timeout=5)
    ...
    def cmd_delete_user(self, chat_id):
        ...
        r = requests.delete(f"{API_BASE}/users/{chat_id}", timeout=5)
        if r.status_code == 200:
    ...
    def cmd_create_greenhouse(self, chat_id, args):
        ...
        name = ' '.join(args)
        gh_list = requests.get(f"{API_BASE}/greenhouses", timeout=5).json().get('GreenHouses', [])
        new_id = max((gh['ID'] for gh in gh_list), default=0) + 1
        payload = {"ID": new_id, "Name": name, "Location": "", "Zones": []}
        r = requests.post(f"{API_BASE}/greenhouses", json=payload, timeout=5)
        if r.status_code == 200:
    ...
    def cmd_delete_greenhouse(self, chat_id, args):
        ...
        r = requests.delete(f"{API_BASE}/greenhouses/{gh_id}", timeout=5)
        if r.status_code == 200:
    ...
    def cmd_create_zone(self, chat_id, args):
        ...
        gh_id = int(args[0])
        name = ' '.join(args[1:])
        zones = requests.get(f"{API_BASE}/zones", timeout=5).json().get('ZonesList', [])
        ...
        r = requests.post(f"{API_BASE}/zones", json=payload , timeout=5) 
        if r.status_code == 200:
    ...
    def cmd_delete_zone(self, chat_id, args):
        ...
        r = requests.delete(f"{API_BASE}/zones/{zone_id}", timeout=5)
        if r.status_code == 200:
    ...
    def cmd_update_moisture(self, chat_id, args):
        ...
        r1 = requests.get(f"{API_BASE}/zones", params={"zoneID": zone_id}, timeout=5)
        ...
        r2 = requests.put(f"{API_BASE}/zones", json=zone, timeout=5)
    ...
    def cmd_list(self, chat_id, args):
        ...
        data = requests.get(f"{API_BASE}/all", timeout=5).json()
